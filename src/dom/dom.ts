...
export function getRect(elem: Element | Document) {
  ...
  if (elem.nodeType === 9) {
    r.width = Math.max((elem as Document).defaultView.innerWidth, html.clientWidth);
    r.height = Math.max((elem as Document).defaultView.innerHeight, html.clientHeight);
  } else {
...
export function on(elem: Element | Document, eventName: string, selector: string | typeof eventHandler, eventHandler?: ((e: Event, sender?: any) => void) | typeof thisArg, thisArg?: any, eventOptions?: any) {
  ...
  const originalFix = eventFix[eventName] || 0 as EventFix;
  const fix = selector && originalFix.delegate ? eventFix[eventName = originalFix.delegate] || 0 as EventFix : originalFix;
...
export function off(elem: Element | Document, eventName: string, selector?: string | typeof eventHandler, eventHandler?: (e: Event, sender: any) => void, thisArg?: any, eventOptions?: any) {
  ...
  if (eventHandler) {
    ...
    // 底层解绑事件。
    const fix = eventFix && eventFix[eventName] || 0 as EventFix;
    if (fix.remove) {
